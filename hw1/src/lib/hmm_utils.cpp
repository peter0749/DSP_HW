...
namespace HMM {
    double hmmForward( HMM *hmm, int *observe, int T, double *alpha )
    {
        ...
        if ( !return_alpha )
        alpha = (double*)malloc( sizeof(double) * s_n * T ); 
        //  assert( alpha != NULL );
        //  Initialize alpha
        for ( int i = 0; i < s_n; ++i ){
    ...
    double hmmBackward( HMM *hmm, int *observe, int T, double *beta )
    {
        ...
        if ( !return_beta )
        beta = (double*)malloc( sizeof(double) * s_n * T ); 
        //  assert( beta != NULL );
        for ( int i = 0; i < s_n; ++i )
    ...
    double hmmViterbi( HMM *hmm, int *observe, int T, int *result )
    {
        ...
        double *delta = NULL;
        int *backtrack = NULL;
        delta = (double*)malloc( sizeof(double) * s_n * 2 ); 
        backtrack = (int*)malloc( sizeof(int) * s_n * T ); 
        //  assert( result != NULL );
        //  assert( delta != NULL );
	...
	// static int cnt_checker = 0; // Check race condition here?!
        ...
        alpha = (double*)malloc( sizeof(double) * s_n * T );   
        beta  = (double*)malloc( sizeof(double) * s_n * T );   
        gamma  = (double*)malloc( sizeof(double) * s_n * T );   
        K = (double*)malloc( sizeof(double) * s_n * s_n * T );   
        ps = (double*)malloc(sizeof(double)*s_n);
        //  assert (alpha != NULL);
        //  assert (beta != NULL);
        //  assert (gamma != NULL);
        //  assert (K != NULL);
